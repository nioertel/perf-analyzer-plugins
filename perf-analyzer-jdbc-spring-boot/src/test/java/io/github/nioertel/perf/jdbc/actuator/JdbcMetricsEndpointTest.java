...
import io.github.nioertel.perf.test.ModifiableSystemEnvironment;
...
class JdbcMetricsEndpointTest {
	...
	@BeforeAll
	public static void setSpringDataSourceSystemEnv() throws Exception {
		// using system env here to ensure standard features of HikariCP in combination with spring boot still work
		ModifiableSystemEnvironment.put("SPRING_DATASOURCE_HIKARI_MINIMUM_IDLE", "1");
		ModifiableSystemEnvironment.put("SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE", "3");
	}
	...
	void actuatorShouldPublishjdbcMetricsForAutoConfiguredDataSource() throws SQLException {
		BDDAssertions.assertThat(autoConfiguredDataSources.get(0).getMinimumIdle()).isEqualTo(1);
		BDDAssertions.assertThat(autoConfiguredDataSources.get(0).getMaximumPoolSize()).isEqualTo(3);
